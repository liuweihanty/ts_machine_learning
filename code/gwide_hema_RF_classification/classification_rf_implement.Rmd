...
#bootstrp the 0s here, write a function to loop through 100 bootstrapped 0s, and apply hyperparemere tuning + traning + testing for each bootstrap, rank the churned-out genes based on how frequent they appear in these bootstrapped trials.
```


training data: gwide_train_balance_CGC
...
#grid search
# Establish a list of possible values for mtry, nodesize and sampsize
mtry1 <- seq(4, ncol(gwide_train_balance_CGC) * 0.8, 2)
nodesize1 <- seq(3, 10, 2)
sampsize1 <- nrow(gwide_train_balance_CGC) * c(0.7,0.8)

# Create a data frame containing all combinations 
hyper_grid1 <- expand.grid(mtry1 = mtry1, nodesize1 = nodesize1, sampsize1 = sampsize1)
...
for (i in 1:nrow(hyper_grid1)) {
    ...
    # Train a Random Forest model
    rf_model1 <- randomForest(formula = TS_status ~ ., 
                          data = select(gwide_train_balance_CGC,-c("chromosome")),
                          mtry1 = hyper_grid1$mtry1[i],
                          nodesize1 = hyper_grid1$nodesize1[i],
                          sampsize1 = hyper_grid1$sampsize1[i])
                          
    # Store OOB error for the model                      
    oob_err1[i] <- rf_model1$err.rate[nrow(rf_model1$err.rate), "OOB"]
}
...
#taking the 
rf1 = randomForest(TS_status ~ .,  
                   data = select(gwide_train_balance_CGC,-c("chromosome")),importance = TRUE,mtry = 8,nodesize = 3,sampsize = nrow(gwide_train_balance_CGC))
plot(rf1) 
print(rf1)
varImpPlot(rf1)
...
```{r}
# Generate predicted classes using the model object
gwide_test_CGC$hits<- predict(object = rf1,newdata = gwide_test_CGC,type = "class") 

#extract the predicted TS genes in the test set
pred_gene <- filter(gwide_test_CGC, hits == 1)["Gene"]


#overlapping between CGC labeled TS and the predicted TS
high_conf_hits <- as.data.frame(pred_gene$Gene[pred_gene$Gene %in% CGC_TS$Gene])
high_conf_hits

#overlapping between Jeremy's 96 genes with the predicted TS
jeremy_genes_overlap <- as.data.frame(pred_gene$Gene[pred_gene$Gene %in% jeremy_genes$Gene])
jeremy_genes_overlap
#29 overlaps

#export predited TS
write.csv(pred_gene,"/Users/weihan/Documents/GitHub/ts_machine_learning/data/gwide_hema_classification/pred_hits_CGC.csv")
```
