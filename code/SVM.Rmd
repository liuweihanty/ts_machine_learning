...
Terminology in SVM parameters:
Gamma: defined how far the influence of a single training example reaches
...
The basic idea of using a more complex kernel is to devise a transformation that makes the problem linearly seperable. The function that transform a polynomial kernel function to a linear kernel function is called "Kernel function"
Exercise
Exercise
...
For polynomial kernels especially, we need to tune the parameters, because it is hard to find optimal values mannually for complex kernels, in a nutshell:
1. set a search range for each paremeters cost, gamma and coef0
2.Build an SVM model for each possible combination of parameter values and evaluate accuracy.
3.Return the parameter combination that yields the best accuracy.

this can be achieved by the tune.svm() function
...
RBF kernel is a very flexible kernel that can fit complex decision boundaries for complex data.RBF kernel simulates KNN
