...
#method 2 non-parametric Mann-Whitney test 
```{r}
...
```{r}
pro.ery.merged.p <- mw_test(pro.ery.merged,0.05)[[1]]
#the significant genes when performing proliferation and erythrocyte score together        
pro.ery.hits <- rownames(mw_test(pro.ery.merged,0.05)[[2]]) 
...
comb_df_norm %>% ggplot(aes(x = avg.pro,y = avg.ery)) + 
        ...
        geom_text_repel(aes(label=ifelse(comb_df_norm$Gene %in% pro.ery.hits | comb_df_norm$Gene == "AAVS1",as.character(Gene),''))) +
